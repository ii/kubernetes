...
var _ = SIGDescribe("Conformance", func() {
	...
	ginkgo.It("should run through the lifecycle of a node", func(ctx context.Context) {
		...
		fakeNode.Name = "e2e-fake-node-" + utilrand.String(5)
		fakeNode.Status.Phase = v1.NodeRunning

		fakeNode.Status.Conditions = append(fakeNode.Status.Conditions, v1.NodeCondition{
		...
		updatedSelector := labels.Set{fakeNode.Name: "updated"}.AsSelector().String()
		...
		err = framework.Gomega().Eventually(ctx, framework.HandleRetry(func(ctx context.Context) (*state, error) {
			nodeList, err := nodeClient.List(ctx, metav1.ListOptions{LabelSelector: updatedSelector})
			if err != nil {
		...
		})).WithTimeout(3 * time.Minute).Should(framework.MakeMatcher(func(s *state) (func() string, error) {
			if len(s.Nodes) == 0 {
				return nil, nil
			}
			return func() string {
				return fmt.Sprintf("Expected node to be deleted, found %q", s.Nodes[0].Name)
			}, nil
		}))
		framework.ExpectNoError(err, "Timeout while waiting to confirm Node %q deletion", fakeNode.Name)
	})
})
