		...
		for _, ref := range pod.OwnerReferences {
			if *ref.Controller {
				if pvcSet, err := e.getPVCSet(pod); err == nil {
					...
					// If error encountered, log error and return nil (i.e. no equivalent pod found)
					glog.Errorf("[EquivalencePodGenerator] for pod: %v failed due to: %v", pod.GetName(), err)
					return nil
				}
			}
		}
